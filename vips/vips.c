...
{
  if (G_IS_OBJECT(*source_custom))
    g_clear_object(source_custom);
}
...
int remove_exif(VipsImage *in, VipsImage **out)
{
  ...
  for (int i = 0; fields[i] != NULL; i++)
  {
    ...
    if (strcmp(name, VIPS_META_ICC_NAME) == 0)
      continue;
    if (strcmp(name, VIPS_META_ORIENTATION) == 0)
      continue;
    if (strcmp(name, VIPS_META_N_PAGES) == 0)
      continue;
    if (strcmp(name, VIPS_META_PAGE_HEIGHT) == 0)
      continue;
    if (strcmp(name, "palette-bit-depth") == 0)
      continue;
